...
export function MatchLobbyWithApiInfo(props: MatchLobbyWithApiInfoProps): JSX.Element {
  const {
    game,
    match: { players, matchID: matchID_ },
  } = props;
  ...
  return (
    <BoxWithLegend legend="Existing Game">
      <MatchLobbyDiv>
        <Names>
          <span>Players:</span> {allNames}
        </Names>
...
export function MatchLobby(props: MatchLobbyProps): JSX.Element {
  ...
  const asyncMatch = useAsync(
    () => lobbyClient.getMatch({ gameName: game.name, matchID: matchID.mid }),
    [],
  );
