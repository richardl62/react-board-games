...
describe('Trie given letter', () => {
  ...
  test.each(testData)('%s', (letterSet, givenLetters, expected) => {
    const found = trie.findWords(
      new WordConstraint(makeLetterSet(letterSet), givenLetters.split('').map(makeConstraint), 0),
    );
...
describe('Trie allowed letter', () => {
  ...
  test.each(testData)('%s %p', (letterSet, allowedLetters, expected) => {
    const found = trie.findWords(
      new WordConstraint(makeLetterSet(letterSet), allowedLetters.map(makeConstraint), 0),
    );
