...
export function Spread(props: SpreadProps): JSX.Element {
  ...
  const separation = Math.min(naturalSeparation(elems.length, props), maxElemSeparation);

  return (
    <OuterDiv width={totalWidth} height={elemHeight}>
      {elems[0]}
      {elems.map((elem, index) => {
        return (
          <Positioned key={index} zIndex={index} left={index * (elemWidth + separation)}>
            {elem}
          </Positioned>
        );
      })}
