...
import { applyPossibleWord } from '../high-scoring-words/apply-possible-word';
import { getHighScoringWords } from '../high-scoring-words/get-high-scoring-words';
...
export function scrabbleReducer(state: ReducerState, action: ActionType): ReducerState {
  ...
  if (action.type === 'setHistoryPosition') {
    return newReducerState(state.scrabbleGameProps, {
      ...state,
      reviewGameHistory: { historyPosition: action.data.position },
    });
  }
...
function newClickMoveState(
  row: number,
  col: number,
  oldCMS: WordPosition | null,
): WordPosition | null {
