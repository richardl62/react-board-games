...
function findCandidateWordsDirected(
  ...
  positions.forEach((rc) => {
    const word = findWordsContaining([rc], board, otherDirection(primaryDirection));
 ...
 */
function findCandidateWords(board: BoardData, active: RowCol[]): RowCol[][] | null {
  return (
 ...
 */
function validWordPosition(board: BoardData, active: RowCol[]): boolean {
...
export function getWordsAndScore(
  context: ReducedScrabbleContext,
  active: RowCol[],
): WordsAndScore | null {
