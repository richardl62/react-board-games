...
function BoardWrapper(props: BoardWrapperProps): JSX.Element {
  ...
  useEffect(() => {
    config
      .getLegalWords()
      .then((legalWords) => setTrie(new Trie(legalWords.map((w) => w.toUpperCase()))))
      .catch(setLoadError);
  ...
  if (JSON.stringify(scrabbleGameProps.G) !== JSON.stringify(reducerState.scrabbleGameProps.G)) {
    dispatch({
  ...
  return (
    <ReactScrabbleContext.Provider value={context}>
      <Board />
