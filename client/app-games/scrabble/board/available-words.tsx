...
export function AvailableWords(): JSX.Element | null {
  ...
  const [wordsToShow, setWordsToShow] = React.useState<{ words: string[]; generatingRack: Rack }>({
    words: [],
    generatingRack: [],
  });
    ...
    <div>
      <div>
        <label>
          ...
          <input type="checkbox" checked={showAvailableWords} onChange={onCheckBoxChange} />
        </label>
      </div>

      {wordsToShow.words.map((word) => (
