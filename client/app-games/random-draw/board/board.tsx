...
function PlayerValues(props: { pid: string }): JSX.Element {
  ...
  return (
    <div>
      ...
      <button onClick={() => moves.draw()} disabled={props.pid !== currentPlayer}>
        Draw
      </button>

      <button onClick={() => events.endTurn()} disabled={props.pid !== currentPlayer}>
        End Turn
      </button>

      <button onClick={() => moves.throwError(undefined)} disabled={props.pid !== currentPlayer}>
        throw Error
      </button>
