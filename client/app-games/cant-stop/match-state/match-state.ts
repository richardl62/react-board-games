...
export interface MatchState extends StandardMatchState {
   ...
   */
  isBlocked: ({ playerID, column, height }: IsBlockedArg0) => boolean;
...
export function useMatchState(): MatchState {
  ...
  const isBlockedSimplified = (arg: IsBlockedArg0) =>
    isBlocked(arg, columnHeights, standardState.G.options);
  const isFullSimplified = (col: number, category: keyof ColumnHeight) =>
    isFull(col, category, columnHeights);

  const state: MatchState = {
    ...standardState,
    isBlocked: isBlockedSimplified,
    isFull: isFullSimplified,
    currentlyBlockedColumns: blockedColumns(columnHeights, playerID, isBlockedSimplified),
  };
