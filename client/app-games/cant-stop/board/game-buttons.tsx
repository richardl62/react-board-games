...
function InnerGameButtons(): JSX.Element {
  ...
  if (scoringChoice === 'rollRequired') {
    return (
      <NoOptionRollOrBustButton onClick={movesDisabled ? undefined : () => moves.roll()}>
        Roll
      </NoOptionRollOrBustButton>
    );
  }
  ...
  if (scoringChoice === 'bust') {
    return (
      <NoOptionRollOrBustButton onClick={movesDisabled ? undefined : () => moves.acknowledgeBust()}>
        Bust
      </NoOptionRollOrBustButton>
    );
  }
  ...
  const blockMessage =
    currentlyBlockedColumns.length > 0
      ? `Blocked on col ${currentlyBlockedColumns.join(', ')}`
      : '';
  return (
    <>
      <RollDontButton onClick={() => moves.roll()} disabled={rollDisabled}>
        Roll
      </RollDontButton>
      ...
      <RollDontButton onClick={() => moves.stopRolling()} disabled={dontDisabled}>
        Don't
      </RollDontButton>
    </>
  );
