...
import {
  CrossTilesContext,
  useCrossTilesContext,
} from '../client-side/actions/cross-tiles-context';
import { ScoreOptions } from '../client-side/check-grid/score-options';
import { scoreCategories, ScoreCategory } from '@game-control/games/crosstiles/score-categories';
...
export function ScoreCards(): JSX.Element | null {
  ...
  const scoreOptions =
    stage === GameStage.scoring ? new ScoreOptions(playerData, isLegalWord) : null;
  ...
  for (const category of scoreCategories) {
    ...
    for (let i = 0; i < nPlayers; ++i) {
      ...
      if (score !== null) {
        const action =
          pid === playerID ? () => moves.setScore({ score, category, bonus }) : undefined;
        elems.push(
          <OptionalScore key={key} onClick={action}>
            {score}{' '}
          </OptionalScore>,
        );
      } else {
        ...
        elems.push(
          <KnownScore key={key} recentlyChosen={chosenCategory === category}>
            {scoreCard[category]}
          </KnownScore>,
        );
      }
