  ...
  return (
    <OuterDiv>
      ...
      {availableActions.rollAll && (
        <button onClick={() => roll('all')} disabled={allButtonsDisabled}>
          Roll All
        </button>
      )}
      {availableActions.endTurn && (
        <button onClick={() => moves.endTurnNotBust()} disabled={allButtonsDisabled}>
          Done
        </button>
      )}
      {availableActions.bust && (
        <BustButton onClick={() => moves.endTurnBust()} disabled={allButtonsDisabled}>
          Bust!
        </BustButton>
      )}
      {G.options.manualDiceRolls && (
        <input
          type="text"
          value={diceText}
