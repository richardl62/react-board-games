...
import { nPreStartPegs } from '@game-control/games/cribbage/config';
...
*/
const PegContainer = styled.div.attrs<HoleProps>((props) => {
  // What is consumed by .attrs()
  return {
    style: {
      position: 'absolute',
      bottom: props.bottom,
...
function makeElements(pegPoints: Position[], props: PlayerProps, player1: boolean) {
  return pegPoints.map((pos, index) => {
    ...
    return (
      <PegContainer key={key} bottom={pos.bottom} left={pos.left} onClick={onClick}>
