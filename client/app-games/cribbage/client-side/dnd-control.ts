...
export function dragAllowed(context: CribbageState, cardSetID: CardSetID, index: number): boolean {
...
export function showBack(context: CribbageState, cardSetID: CardSetID, index: number): boolean {
...
export function dropTarget(context: CribbageState, cardSetID: CardSetID, index?: number): boolean {
  const { stage, pone, numPlayers } = context;
  const singlePlayer = numPlayers === 1;
  ...
  if (stage === GameStage.SettingBox) {
    ...
    return cardSetID === CardSetID.Shared && index === undefined && !boxFull(context, context.me);
  }
