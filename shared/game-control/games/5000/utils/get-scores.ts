...
export function getScores({
  ...
  if (scoreOnlyThreePairs.scores.threePairs > 0) {
    if (totalScore(scoreOnlyThreePairs.scores) >= totalScore(scoreExcludingThreePairs.scores)) {
...
function checkAllSame(sortedFaces: number[], scores: DiceScore, allSameScore: number) {
  if (sortedFaces.length === 6 && sortedFaces.every((face) => face === sortedFaces[0])) {
