...
type PassParam = void;
function pass({ G: state, ctx }: MoveArg0<GameState>, _param: PassParam): void {
...
function nextPlayer(ctx: Ctx) {
  ...
  sAssert(currentPlayer === playOrder[playOrderPos], 'currentPlayer mismatch in context');
...
function customWrappedMoveFunction<P>(func: SimpleMoveFunc<P>): WrappedMoveFunc<P> {
  return (arg0, param) => {
    ...
    sAssert(
      currentState.currentPlayer === ctx.currentPlayer,
