...
export class SharedPile {
  ...
  stealTopCard(thiefCard: CardNonJoker): CardNonJoker {
    ...
    const stolen = this._thisTurnStandard.pop() ?? this._old.pop()!;
  ...
  addStandardCard(card: CardNonJoker) {
    ...
    sAssert(
      card.rank === 'K' || !optionsWrapper.isSpecial(card),
...
export function makeSharedPiles({
  sharedPileData,
  options,
}: {
  ...
  return sharedPileData.map((data) => new SharedPile(data, options));
}
