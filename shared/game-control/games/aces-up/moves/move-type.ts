...
export function moveType(
  ...
  const isDiscardPile = (card: CardID) =>
    card.area === 'discardPileAll' || card.area === 'discardPileCard';
  ...
  if (
    to.area === 'discardPileAll' &&
    from.area === 'hand' &&
    isCurrentPlayer(to) &&
    isCurrentPlayer(from)
  ) {
    return 'move';
  }
  ...
  if (isDiscardPile(from) && isDiscardPile(to)) {
    return isCurrentPlayer(from) &&
      isCurrentPlayer(to) &&
      discardPileIndex(from) !== discardPileIndex(to)
      ? 'move'
      : null;
  }
  ...
  if (isDiscardPile(to)) {
    return from.area === 'hand' && isCurrentPlayer(from) && isCurrentPlayer(to) ? 'move' : null;
  }
